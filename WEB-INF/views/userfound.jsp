...
<head>
<meta charset="EUC-KR">
<title>Lost And Found - Found</title>
...
<body onLoad="pageOnLoad('')">
	<div id="wrap" style="position: relative; z-index: 1;">
		...
		<!-- Banner End -->
		<!-- Content -->
		<div id="found" style="margin: 2%; height: auto; display: block">
			<div
				style="display: flex; justify-content: space-between; align-items: center;">
				<h1 style="width: 55%;">
					<div id="lftitle" style="width: 50%; margin-bottom: 5%;">습득물
						목록</div>
				</h1>
			</div>
			<!-- DataTales Example -->
  ...
  <div style="display: flex; flex-direction: column; align-items: center; max-width: 60vw;">
    <div id="drop_zone" style="height: 20vw; display: flex; justify-content: center; align-items: center; max-width: 100%;">
    ...
    <div style="text-align: center;">
      <button type="button" class="btn btn-primary" onClick="imgSearchCtl(2)">이미지 첨부</button>
      <button type="button" class="btn btn-primary" onClick="imgSearchCtl(3)">이미지 검색</button>
      <button type="button" class="btn btn-primary" onClick="imgSearchCtl(0)">닫기</button>
    </div>
  </div>
</div>
		</div>
		<div class="messagebackground search"
				...
				<div id="noticebody" style="height: 45vw">
					<section class="about-cards-section">
						<div class="container">
							<div class="row">
								<div class="col-sm-1 search-wrapper"
									...
									<div class="card border-0">
										<div
											class="position-relative overflow-hidden mx-auto custom-search-image">
							...
							<div class="row">
								<div class="col-sm-6 search2-wrapper">
							...
							<div class="row">
								...
								<div class="col-sm-6 search2-wrapper">
									<div class="form-group">
										<label class="form-label-sm-2">습득물명 </label> <input
											class="form-control-sm-4" id="sfaname" type="text"
							...
							<div class="row">
								<div class="col-sm-6 search2-wrapper">
									<div class="form-group">
										<label class="form-label-sm-2">보관장소 </label> <input
											class="form-control-sm-4" id="sdepplace" type="text"
											placeholder="" style="width: 70%">
									</div>
								</div>
								<div class="col-sm-6 search2-wrapper">
									<div class="form-group">
										<label class="form-label-sm-2">습득장소 </label> <select class="plsel" name="place">
							...
							<div class="row">
								<div class="col-sm-6 search2-wrapper">
									<div class="form-group">
										<label class="form-label-sm-2">색상 </label> <input
											class="form-control-sm-4" id="scolor" type="text"
											placeholder="" style="width: 70%">
									</div>
								</div>
							</div>
	...
	function updateCarouselPosition() {
		  ...
		  var carouselTop = 85; // 새로운 top 값
		  if (scrollPosition > 100) { // 스크롤 위치가 100 이상이면
		    carouselTop = scrollPosition + 85; // 스크롤 위치에 따라 carousel 위치를 조정
		  }
	...
	function foundcomplatecomment(jsonData){
		if(jsonData.faCommentList != null && jsonData.faCommentList != ''){
			...
			for(let i=0; i<serverData.falist.length ; i++){
				if(serverData.falist[i].faCtNumber == jsonData.faCommentList[0].fcoCode){
					serverData.falist[i].faCommentList=jsonData.faCommentList;
				}
			}
	...
	function tbodyReplace(){
		...
		for(let i=0; i<serverData.falist.length; i++){
			...
			tr.setAttribute("onclick","moveDetail('"+serverData.falist[i].faCtNumber+"')");
			if(serverData.falist[i].faImgList != ''){
				tr.setAttribute("style","color:#337AB7");
			}
			for(key in serverData.falist[i]) {
			    if(key == 'faName'){
			    	const td=document.createElement("td");
			    	td.append(serverData.falist[i].faName);
			    	tr.append(td);
			    }else if(key == 'faDate'){
			    	const td=document.createElement("td");
			    	td.append(serverData.falist[i].faDate);
			    	td.setAttribute("tabindex",1);
			    	tr.prepend(td);
			    }else if(key == 'faPlace'){
			    	const td=document.createElement("td");
			    	td.append(serverData.falist[i].faPlace);
			    	tr.append(td);
			    }else if(key == 'faDepPlace'){
			    	const td=document.createElement("td");
			    	td.append(serverData.falist[i].faDepPlace);
			    	tr.append(td);
			    }
			}
	...
	function pageClick(){
		...
		if(serverData.pageType=='T')
			serverCallByFetchAjax(formData, '/Api/getUserFoundByPageNo', 'post', 'pageCallBack');
		else if(serverData.pageType=='S')
		{
			...
			if(serverData.sinfo.color != null)
				formData.set("sInfo.color",serverData.sinfo.color);
			if(serverData.sinfo.name != null) 
				formData.set("sInfo.name",serverData.sinfo.name);
			if(serverData.sinfo.depPlace != null)
				formData.set("sInfo.depPlace",serverData.sinfo.depPlace);
			...
			serverCallByFetchAjax(formData, '/Api/getFoundDetailSearch', 'post', 'pageCallBack');			
		}else if(serverData.pageType=='I')
		{
			...
			serverCallByFetchAjax(formData, '/Api/ImageSearchFF', 'post', 'pageCallBack');
		}
	}
	...
	/*function kakaoPostCodeAPI() {
	    ...
	    new daum.Postcode({
	        oncomplete: function(data) {
	           ...
	           document.getElementById('roadsearch').value =  data.roadAddress
	           document.getElementById('roadsearch').focus();
	        }
	    }).open();
	}*/
	function moveDetail() {
	...
	function imgSearchCtl() {
		if (arguments[0] == 0) {
			...
			document
					.getElementsByClassName('messagebackground imagesearch')[0].style.display = 'none';
		} else if (arguments[0] == 1) {
			document
					.getElementsByClassName('messagebackground imagesearch')[0].style.display = 'block';
		}else if (arguments[0] == 2) { //첨부
			...
			if (image) {
			  ...
			  canvas.toBlob(function(blob) {
			    ...
			    serverCallByFetchAjax(formData, '/Api/ImageSearchF', 'post', 'pageCallBack');
			    imgSearchCtl(0);
			    LoadingSpinner(1);
			  }, filetype, 0.9);
			}
	  ...
	  if (files.length === 1 && files[0].type.startsWith("image")) {
		...
		function aCtl(){
			if(arguments[1].length==2){
				...
				/*document.getElementById('mcategoryname').value=document
				.getElementsByClassName('categorydetail')[arguments[0]-(parseInt(arguments[1].substring(2,3))+1)
					].innerText;
				document.getElementById('mcategorycode').value=	parseInt(arguments[1].substring(2,2))+1;*/
			}
		}
		function searchDetailCtl(){
			if(arguments[0]==1){
				...
				if(document.getElementsByClassName("plsel")[0].value != '')
					formData.set("sInfo.place",document.getElementsByClassName("plsel")[0].value);
				if(document.getElementById('scolor').value != ''){
					let color=document.getElementById('scolor').value;
					if(color.indexOf('색') != -1){
						formData.set("sInfo.color",color.substring(0,color.indexOf('색')));
					}else{
						formData.set("sInfo.color",color);
					}
				}
				if(document.getElementById('sfaname').value != '') 
					formData.set("sInfo.name",document.getElementById('sfaname').value);
				if(document.getElementById('sdepplace').value != '')
					formData.set("sInfo.depPlace",document.getElementById('sdepplace').value);
				...
				document.getElementsByClassName("plsel")[0].value='';
				document.getElementById('scolor').value='';
				document.getElementById('sfaname').value='';
				document.getElementById('sdepplace').value='';
		...
		function searchDetailInit(){
			...
			document.getElementsByClassName('plsel')[0][0].selected=true;
			document.getElementById('scolor').value='';
			document.getElementById('sfaname').value='';
			document.getElementById('sdepplace').value='';
			document.getElementsByClassName('datepicker sm-2')[0].value='';
