...
<head>
<meta charset="EUC-KR">
<title>Lost And Found - LostReg</title>
...
<body onLoad="pageLoad('')">
	<div id="wrap" style="position: relative; z-index: 1;">
		...
		<!-- Banner End -->
		<!-- Content -->
		<div id="reglost" style="display: block">
			<form id="regform">
				<div id=regloststep1 style="display: block">
					<h1>Report Lost</h1>
					...
					<fieldset id="regLost">
						...
						<label for="formFile" class="regimage">* 물품 사진 </label> <input
							class="form-control" type="file" id="regimg">
					</div>
						<div class="form-group">
							<label for="exampleSelect1" class="form-label mt-4">* 분실
						...
						<div class="form-group">
							<label for="exampleTextarea" class="form-label mt-4">* 분실
							장소 도로명 주소(주변)</label>
						<textarea class="form-control" id="reglostroad" rows="1" onClick="kakaoPostCodeAPI()">
						</textarea>
				...
				<div id=regloststep2 style="display: block">
					<div class="form-group">
						<label class="col-form-label mt-4" for="inputDefault">* 분실
							날짜</label><br><input type="date" id="ladate" name="date"><br>
						<label for="exampleSelect1" class="form-label mt-4 width50">
							* 분실 시간<select class="form-select " id="reghour">
					...
					<div>
						<label class="col-form-label mt-4" for="inputDefault">* 글
							제목</label> <small>(분실물 목록에 표시되는 제목입니다.)</small>
							</div>
							<div>
					...
					<div>
						<label for="exampleSelect1" class="form-label mt-4 width50">*
							대분류<select class="form-select " id="MainCategories" name="MainCategories"
				...
				<div id=regloststep3 style="display: block">
					<div class="form-group">
					...
					<div class="form-group">
						<label for="exampleTextarea" class="form-label mt-4">* 상세
							내용</label>
						<textarea class="form-control" id="reglostdetail" rows="3"></textarea>
					</div>
					<div style="width: 100%; text-align: center; padding: 10%">
						<div style="width: 30%" class="btn btn-primary"
							onclick="userLostRegister()">등록</div>
					</div>
				</div>
				</fieldset>
			</form>
	...
	function userLostRegister() {
		let formData = new FormData();
		let keywordList =[];
		keywordList.push(document.getElementsByClassName("losel")[0][document.getElementsByClassName("losel")[0].selectedIndex].textContent.substring(0,2));
	    ...
	    formData.set("mInfo.plCode",document.getElementsByClassName("plsel")[0].value);
	    ...
	    formData.set("mInfo.date",document.getElementById("ladate").value);
	    //serverCallByFetchAjax(formData, '/Api/lostMatching', 'post', 'pageCallBack');
	    
	    formData.set("lalist[0].laMcCode",document.getElementById("MainCategories").value);
	    formData.set("lalist[0].laUcCode",document.getElementById("SubCategories").value);
	    let laName='';
	    let regtags=document.getElementById("tag-container-2").querySelectorAll(".tag");
	    for (let i=0; i<regtags.length; i++){
	    	if(i>0) laName+=' ';
	    	laName+=regtags[i].textContent.split('×')[0];
	    }
	    let laPlace=document.getElementsByClassName("plsel")[0][document.getElementsByClassName("plsel")[0].selectedIndex].textContent;
	    let regtags2=document.getElementById("tag-container-1").querySelectorAll(".tag");
	    for (let i=0; i<regtags2.length; i++){
	    	laPlace+=regtags2[i].textContent.split('×')[0];
	    }
	    
	    formData.set("lalist[0].laName",laName);
	    formData.set("lalist[0].laPlace",laPlace);
	    formData.set("lalist[0].laDate",document.getElementById("ladate").value);
	    formData.set("lalist[0].laStatus",'O');
	    formData.set("lalist[0].laLocation",document.getElementsByClassName("losel")[0].value);
	    formData.set("lalist[0].laUnique",document.getElementById("reglostroad").value+':주변주소 , 상세내용:'+document.getElementById("reglostdetail").value);
	    formData.set("lalist[0].laHour",document.getElementById("reghour").value);
	    
	    serverCallByFetchAjax(formData, '/Api/userLostRegister', 'post', 'pageCallBack');
	...
	document.getElementById("regimg").addEventListener("change", (event) => {
		  ...
		  if (file.type === "image/gif") {
		    ...
		    alert("Gif 가 아닌 정적 이미지를 올려주세요");
				...
				serverCallByFetchAjax(formData, '/Api/userLostBefore', 'post', '');
		  }
		});
	function regComment(){
	...
	function bannerDetail() {
		...
		if(arguments[0]!=0){
			...
			if(document.getElementById("reglost").style.display == 'block'){
				document.getElementById("reglost").style.display = 'none';
				status=1;			
			...
			if(status==1)
				document.getElementById("reglost").style.display = 'block';
			else if(status==3)
...
window.addEventListener('scroll', function() {
	...
	function moveDetail() {
		...
		if(arguments[0]!=0){
			...
			if(serverData.falist[dno].member.mmSns=='AG')
				document.getElementById("faPostId").innerText += serverData.falist[dno].faDepPlace.split('&')[0];
			else
			...
			if(serverData.falist[dno].faUnique.indexOf("&") != -1){
				document.getElementById("faPlace").innerText += +","+serverData.falist[dno].faPlace+serverData.falist[dno].faUnique.split("&")[1];
			}
			else
			...
			if(serverData.falist[dno].member.mmSns=='AG')
				...
				document.getElementById("faOrgId").innerText += serverData.falist[dno].faMmId.substring(0,3)+'***';
			
			if(arguments[0][0]=='S'){
				let un=serverData.falist[dno].faUnique.replace(/<br>/g, "\n");
				if(serverData.falist[dno].faUnique.indexOf("&") != -1){
					document.getElementById("faUnique").innerText += un.split("&")[0];
				}
				else document.getElementById("faUnique").innerText += serverData.falist[dno].faUnique.replace(/<br>/g, "\n");
			}
			else{
				if(serverData.falist[dno].faUnique.indexOf("&") != -1){
					document.getElementById("faUnique").innerText += serverData.falist[dno].faUnique.split("&")[0];
				}else
				{
			...
			foundcomplatecomment(serverData.falist[dno]);
			
			let inputBox = document.getElementById("keyword");
			let depPlace = serverData.falist[dno].faDepPlace;
			  if (depPlace.indexOf('&') !== -1) {
				  depPlace = depPlace.split('&')[0];
			  }
				if(serverData.falist[dno].member.mmSns=='AG')
	...
	function createFoundList(foundArticleList) {
		  const noListMessage = '<div class="alert alert-warning">조회된 분실물이 없습니다.</div>';
		  if (foundArticleList.length === 0) {
		    return noListMessage;
		  }
		  const row = document.createElement('div');
		  row.classList.add('row', 'row-cols-1', 'row-cols-md-3', 'g-4', 'py-3');
		  row.setAttribute('style','width: 30%;text-align: center;position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);');
		    ...
		    cardTitle.innerText = '매칭된 습득물 리스트';
		    cardBody.appendChild(cardTitle);
		    card.appendChild(cardBody);
		    cardCol.appendChild(card);
		    row.appendChild(cardCol);
		    
		  foundArticleList.forEach((foundArticle) => {
		    const cardCol = document.createElement('div');
		    cardCol.classList.add('col');
		    const card = document.createElement('div');
		    card.classList.add('card', 'h-100');
		    card.setAttribute("onclick","moveDetail('"+foundArticle.faCtNumber+"')");
		    card.setAttribute("style","cursor:pointer;");
		    const cardBody = document.createElement('div');
		    cardBody.classList.add('card-body');
		    const cardTitle = document.createElement('h5');
		    cardTitle.classList.add('card-title');
		    cardTitle.innerText = foundArticle.faCtNumber;
		    cardBody.appendChild(cardTitle);
		    card.appendChild(cardBody);
		    cardCol.appendChild(card);
		    row.appendChild(cardCol);
	...
	function pageCallBack(jsonData){
		...
		if(jsonData!='' && jsonData!='error'){
			if(serverData.falist != null && serverData.falist != '')
				{
				document.getElementById("reglost").style.display = 'none';
				document.getElementById('matchingresult').innerHTML = createFoundList(serverData.falist);
				document.getElementById("matchingresult").style.display = 'block';
				}
			else
				moveService('User');
	...
	function kakaoPostCodeAPI() {
	    new daum.Postcode({
	        oncomplete: function(data) {
	           document.getElementById('reglostroad').value =  data.roadAddress
	           document.getElementById('reglostroad').focus();
	        }
	    }).open();
	}
